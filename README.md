...
多渠道选题优先由三个独立任务分渠道寻找，其中一个兼任统筹。每组至少交付两条合格新材料，最终至少六条；未完成持续寻找，不设搜索篇数或时长上限。各组独立执行，完成后再集中交接，统筹复核后只交付一份筛好的审核页。优先最新材料，长期对话结合共享历史去重。Ego 支持按需大量多开，不预设窗口上限；超过一百个相关进程或发生实际资源故障时清理自己的旧页面，保留证据与用户数据。
