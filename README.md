...
默认沿用常规选题流程，以已有来源、聚合检索、RSS、公开网页和可用 CLI 为主。Ego Browser 只在常规路径不足或有价值的页面确需浏览器时作为补充，不是每轮必做步骤；也不默认启动三个 session 或要求每组两条。选路兼顾合格材料产出、耗时和成本，最终只交付筛好的审核页，质量、时效与去重标准不变。
...
本项目不使用 OpenCLI，避免 Browser Bridge 调试或抢占用户的 Google Chrome。需要浏览器补充时使用 Ego Browser 隔离任务空间；常规检索不要求打开浏览器。
