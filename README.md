...
- `resources/content_curator_sources.json`：订阅清单，按类别标注；每个源都抓正文、都按同一套标准判定，没有“只作线索”的源；OpenAI（含 Developers）、Anthropic、Claude、Google DeepMind 的官方博客标了 `official_release`，发布后 5 天内排在最前面。
- `resources/source_portfolio.json`：12 类取材渠道和权重，收工检查用。

脚本（按流程）：
- `scrape_aihot.py`：抓取总入口；`source_audit.py`：逐个源体检；`transcribe_podcasts.py`：挑播客本机转写成逐字稿；`harvest_leads.py`：给订阅覆盖不到的材料补正文登记成候选；`browser_fetch.py`：抓不到的正文用 ego-browser 兜底；`skill_version.py`：确认跑批用的是最新 Skill；`curator.py`：一票否决和阅读顺序；`published.py`：对照已发布文章标出同名候选；`buzz.py`：找出最近 3 天在很多来源同时刷屏的新名字，列在标题清单开头。
