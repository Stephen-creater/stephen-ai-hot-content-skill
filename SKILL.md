...
- 在原始展示页检查正文是否完整；出现“登录后查看剩余内容”等截断提示时直接淘汰，即使代理、缓存或转载能抓到更多文字也不能冒充公开完整材料。
...
- 长播客、完整逐字稿和密集案例只证明材料够长，不证明信息密度高；找不到至少一个足以独立成文的新判断时淘汰。
- 用户明确点了“不入选”但没有备注时，仍按最终否决入库；不能因缺少解释擅自降级成待定。
