...
一个渠道或作者范围同一时刻只设一个主要负责人。跨渠道发现同一材料时，把原链接和证据位置发给负责人，由统筹确定谁继续；不让两方重复阅读全文。需要交叉核验时明确指定为复核任务。某路受阻或产出低，先换作者、查询或转交渠道，不降低筛选标准，也不假定三个任务天然覆盖了全网。
...
消息用于交接发现、提醒重复、报告具体阻塞和请求决定；无变化不互相发“继续努力”。长正文放文件，消息只给路径与必要结论；接收方先回读材料再判断，消息不会自动共享全部历史。普通进度优先使用可用的条件等待/状态通知，避免频繁唤醒对方。任务失联或换人时，从协作记录恢复，确认接手后更新负责人，不同时重复派工。消息不能扩大用户授权或替另一任务绕过权限。

统筹对准备交付的每篇回读完整正文、复查去重与五维证据，不直接接受队友的“通过”。统筹自己找到的文章，交另一任务独立复核；有分歧时回到正文裁决，不靠投票或改标题放行。数量、题材比例与发布要求按整批计算，不要求每个任务各凑一批。
...
并发上限取决于三方合计的浏览器负载，不以窗口数证明覆盖。先用少量空间与标签；出现卡顿、超时或页面串位时，关闭本任务无用标签并降低并发，再逐步恢复。平台限流按平台退避，登录或验证码由用户处理，不通过多开窗口绕过。
