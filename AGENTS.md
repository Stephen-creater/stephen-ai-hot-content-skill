...
- 高优先级完成条件：每批选题按 Goal 模式持续执行，至少找到 5 个符合全部质量标准的全新材料并核验交付后才可结束。不足时继续扩源与全文筛选，不得以“本轮未找到”提前结束或让用户再次催“继续”。旧约定中的“允许 0 条”仅指内部单轮结果，不代表整批任务可以停止。用户主动暂停/取消及真实权限、安全边界仍须遵守。
