...
1. 运行常规来源与本地 inbox。
2. 合格候选不足 3 条时，从尚未覆盖的层级选择至少两个渠道继续搜索。
3. 找到线索后先读取正文、Show Notes、字幕或完整讨论，再写入 `.local/source_inbox.json`。
4. 重新抓取、硬门槛筛选和历史去重。
5. 人工检查所有候选，拦截通稿、SEO 内容、标题党、泛泛观点和过深实现细节。
6. 仍不足 3 条则回到第 2 步，不得交付当前批次。
