...
**浏览器默认就是 Ego Browser**：国内站点走本机直连，海外站点走代理，两边都能打开，不用挑浏览器也不用手工切网络。

它不是“最后才用”的渠道：X 每批必跑靠它；直连和网页读取取不到正文时，抓取脚本自己会调它兜底（`browser_fetch.py`）；需要登录或前端渲染的页面也走它。

- 使用 Ego Browser 的独立任务空间。不启动、不操作你日常用的 Chrome，不导出 Cookie，不替你登录。
...
**中文作者巡检已经自动化**：抓取脚本里的“X 中文作者巡检”源（`scripts/x_sweep.py`）按 `resources/x_authors.json` 的名单逐个翻主页，收最近 48 小时的中文长帖并取全文，结果缓存 3 小时。名单是写中文 AI 内容的作者，加上历史上抓到过中文长帖的作者；新发现写得好的中文作者加进名单，连续几周一条没过的删掉。英文研究员和官方账号不进名单，他们的帖子会被英文正文否决。

除了巡检，每批还要做这三件事，做完把查询词和结果数记进 `discovery_ledger.py record`：
...
- **注意自动翻译**：这个账号的界面不是中文，X 会让 Grok 把中文帖自动翻成英文显示（2026-09-23 巡检第一次跑，30 条全是英文译文）。取正文前先点帖子里的“Show original / 显示原文”切回原文，`x_sweep.py` 已经这样做；只切换显示，不改账号的语言设置。原文语言以“显示原文”为准。中文长帖才算中文材料。界面语言不一定是中文（实测这个账号是葡萄牙语，“显示原文”写作 Mostrar tradução），所以按结构取元素，不要按按钮文字找。
