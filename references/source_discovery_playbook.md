...
- 定期跑 `.venv/bin/python3 scripts/source_audit.py --problems`：每个源实际抓几条、取正文、按一票否决判一遍，报出「取到几条、有正文几条、合格几条、卡在哪」。名单里写着却不出货的源，要么修，要么去掉。2026-09-21 全量体检：199 个源，正常出货 112，平台短内容 22（X、即刻，只有长帖够格），更新慢 18（最新一篇超过 45 天），其余要修。
