# 信息源清单

合格材料不够本批目标条数时，按下面的顺序扩源，直到满足 `SKILL.md`「交付规则」里的条数或停止条件。扩源是为了找到更多好材料，不是降低标准。

每个来源的地址、分组和参数以 `resources/content_curator_sources.json` 为准，这里只讲每一层收什么、怎么用。扩源前后看搜索记录：

```bash
.venv/bin/python3 scripts/discovery_ledger.py report
.venv/bin/python3 scripts/source_yield.py --min-decided 5   # 每个来源被 Stephen 选中几次
```

`resources/source_portfolio.json` 把取材渠道分成 12 类并给了权重，收工检查用它判断高权重的渠道类别有没有搜过。

## 五层来源

历史入选集中在少数访谈和编译号、精选站、一线作者，资讯媒体几乎不出货；不少入选访谈的上游是英文播客或演讲，中文稿是编译或整理。所以按作者和栏目订阅，不扩综合媒体。

### 第一层：中文精选主入口（候选源）

WayToAGI 知识库精选、BestBlogs、觉醒AI、53AI 知识库、宝玉。别人已经筛过一遍的高质量中文内容。觉醒AI 是历史通过率最高的源（42 条过 9 条），也大量把 X 帖子、英文官方博客、演讲视频改写成中文，等于替我们把英文线索换成了中文稿。

- WayToAGI（`waytoagi.com/zh`）每天一页「知识库精选」，六到八篇人工挑的中文文章，和 X 同级别。抓取脚本自动取最近 3 天：知识库页写全了就用它，只放导读的跟着「原文链接」去原文（跟过去以后和公众号源用同一个链接，自然查重）。原文在微信的会先被验证页挡住，由 ego-browser 兜底取回。

- BestBlogs 的评分和摘要是它自己的 AI 生成的，只决定先读什么，交付前回原文核对。
- 觉醒AI 一天发 250 到 700 篇。2026-09-23 起抓“每日发布清单”（`/ai/updates/年/月/日/`，最近两天、每天翻到没有新文章为止），另抓“访谈对话”标签前两页。以前只抓文章库前 60 条，而文章库按更新时间排，只看到当天新文章的一两成。页面里的发布日期（datePublished）才是真的；旧的 X 帖子重新导入时，按正文提到的模型版本核对年代。
- 53AI 知识库（`53ai.com/news/`）转载 AI 公众号全文并注明作者，网页直连可读，绕开微信的验证页。只抓大模型、个人提效两个栏目；它也转载量子位等已有来源，按正文查重。要用完整浏览器 UA（抓取脚本 `BROWSER_UA_HOSTS`）。
- 人人都是产品经理抓 AI 分类的 RSS 并翻 4 页（`extra_urls`），全站 RSS 15 条只覆盖一两个小时。
- Anthropic 的模型发布页不在 /news 下（Opus 5.5 在 `/claude-opus-5-5`），用“盯网站地图新增网址”的源（`sitemap_watch`）抓，第一次运行只记账。
- AI News Radar（LearnPrompt）每天 300 多条，以英文科技新闻为主，主要当“今天什么热”的线索用；抓取上限已放到 600 条，不再截断。
- 宝玉的订阅只有摘要，抓取时回原页取全文。

### 第二层：访谈与实践公众号（候选源）

公众号按组订阅：访谈与文字稿、一线实践与方法、大厂技术团队 Agent 实践、独立作者等。组内共用默认参数（全文、优先级、标题排除规则），单个号只写与默认不同的地方；单号的排除规则追加在组规则之后。

- 大厂技术团队的号大多写底层基建，组规则只收 Agent 落地、AI Coding、评测方法类标题。
- 按号判断价值，不按平台：同一个号的访谈可能入选，活动报名和资讯不入选，用标题排除规则过滤。
- 人人都是产品经理、公众号作战室索引作补充；作战室索引只保留访谈、实录、复盘和长期实测标题。

### 第三层：播客变文字稿（有文字稿才算候选）

按顺序取材，前一步拿到就停：节目方或公众号的中文文字版 → BestBlogs 播客转录 → 本机离线转写。具体做法见 [取材渠道手册](channels.md)。没有可读文字稿的音频只算线索。

中文播客订阅主要走小宇宙，通过公共 RSSHub 的 `/xiaoyuzhou/podcast/<ID>`，订阅里带节目介绍和音频地址。

### 第四层：X 与英文一手

每个源都抓正文，每条材料都按同一套编辑标准判定：够不够新、有没有干货、读者读完能多做什么。英文正文会被抓取脚本否决（2026-09-22 起：7 条英文候选全被拒，理由是改写成本高），OpenAI、Anthropic、Google 的官方发布稿除外。其他英文源只用来发现当天的事，候选要换成讲同一件事的中文稿。

X 是和中文精选站、公众号同等重要的主力渠道，每批都要用 ego-browser 实际去看：首页时间线、重点作者主页、关键词搜索。看到的长帖登记成候选。做法见 [取材渠道手册](channels.md) 的“X（推特）”。订阅里的两个 X 源只覆盖 25 个固定账号、只留最近 24 小时，是补充，不能替代实际去看。

主流厂商的官方发布（OpenAI 含 Developers、Anthropic、Claude、Google DeepMind）标了 `official_release`：发布后 5 天内排在最前面。OpenAI 官网拒绝脚本请求，标了 `reader_fallback`，直连失败时改用网页读取，再不行由 ego-browser 兜底。

- 命中后依次找中文成熟稿：宝玉、Web3天空之城、Z Finance、海外独角兽、BestBlogs 关键词订阅。都没有，就在缺口报告里写“中文材料缺位”，由 Stephen 判断要不要原创。
- 部分英文节目带免费逐字稿，可以用来核对中文编译稿是否忠实。
- YouTube 官方频道订阅对部分频道返回 404，统一走 RSSHub `/youtube/channel/<频道ID>`，命中后用 yt-dlp 取字幕。

### 第五层：独立博客低频池（候选源）

中文个人技术和产品博客。更新慢，但多是一线复盘；非 AI 文章由一票否决过滤。

### 公共 RSSHub

`rsshub.bestblogs.dev` 为主，`rsshub.imlg.co` 为备（配置里的 `mirror_hosts`，主实例失败时按同一路径切换）；`rsshub.app` 官方实例返回 403。能用的路由：小宇宙、YouTube、即刻、Telegram、掘金、少数派、知乎热榜。B 站用户、知乎个人、X、小红书路由需要 Cookie，公共实例不可用。

### 不再接入

- 以资讯为主：36氪、虎嗅、少数派、IT之家、Solidot、Readhub、机器之心、新智元。
- 需要 Cookie 或已不可用：公共 RSSHub 的知乎和 X 路由、nitter/xcancel、搜狗微信、feeddd、wewe-rss。
- 已停更或失效：OnBoard!、AI局内人、海外独角兽播客、xyzfm 托管的播客、证书过期的一批 `.cn` 个人博客。
- 连续 3 批零入选的来源要么修取材方式，要么去掉，依据 `source_yield.py` 和 `source_audit.py`，不凭印象增删。
- 定期跑 `.venv/bin/python3 scripts/source_audit.py --problems`：每个源实际抓几条、取正文、按一票否决判一遍，报出「取到几条、有正文几条、合格几条、卡在哪」。名单里写着却不出货的源，要么修，要么去掉。2026-09-21 全量体检（198 个源）：

| 状态 | 数量 | 说明 |
|---|---|---|
| 正常出货 | 115 | 抓到条目、取到正文、样本里有合格的 |
| 播客源 | 26 | 有音频没逐字稿，用 `transcribe_podcasts.py` 本机转写后即可判定 |
| 平台短内容 | 19 | X、即刻、Telegram 的单条短帖，只有长帖够格 |
| 更新慢 | 18 | 最新一篇超过 45 天，源没坏，是作者不更新 |
| 样本全被否决 | 12 | 正文都抓到了，是这批内容不符合标准（多为非 AI 内容或超时效） |
| 取不到正文 / 被站点拦 | 8 | 逐个查：站点挂掉的从名单删除（XINDOO 整站 500 已删），被反爬的由 ego-browser 兜底 |
- 体检和抓取共用 HTTP 缓存，并发默认 3：本机代理并发高了会拒连，那会把网络抖动误记成“这个源抓不到”。
- 一个源“看到很多、合格为零”，先看是不是取材坏了（站点返回验证页、整站报错），再下结论说内容不行。

## 接入新源

Stephen 给 OPML 或订阅清单时，按实测数据逐个评级接入，不整包导入：

1. 去掉已接入的地址，丢弃只属于生活、理财、前端、移动端、后端和运维分类的源。
2. 低并发探测（wechat2rss 并发高了会超时）：是否可用、近 45 天篇数、AI 相关篇数、正文不少于 800 字的篇数。近 45 天 AI 全文少于 2 篇的不评。
3. 按近 45 天标题逐个评级，参照编辑判断标准和 `source_yield.py` 列出的真实入选和淘汰例子：A 级（至少 3 篇像会入选的）按组默认值接入；B 级（1 到 2 篇）优先级 3、条数少于组默认；C 级按优先级 2 接入或不接；X 级不接。
4. 排除规则只写反复出现的噪音栏目或事件类型，例如早报、报名、大会、招聘；不要拿某一篇标题里的字眼当规则，否则以后的好文章会被误伤。
5. 试抓一轮，看新来源带进来多少合格材料、多少噪音；噪音集中在某类标题时，回到第 4 步补规则。

## 搜索组合

围绕这些结构交叉搜索，而不是只搜“AI 热点”：

- `AI Agent + 一个月/半年/一年 + 复盘/失败/踩坑`
- `Claude Code/Codex + 正式项目 + commits/测试/交付`
- `AI + 学习/记忆/认知/创造力 + 实证研究`
- `Agent + 真实工作流 + 数字/成本/成功率/留存`
- `AI + 组织变化 + 岗位/流程/协作 + 案例`
- `访谈/播客/逐字稿 + 核心团队 + 具体问题`

## 扩源循环

1. 先跑订阅源和手动补货单。
2. 不够时，组合 Exa、公开文章索引、网页读取和 B 站或 YouTube 字幕；还不够，再用浏览器渠道查知乎和必须登录的页面。X、Reddit、V2EX、GitHub 只负责发现线索。各渠道用法见 [取材渠道手册](channels.md)。
3. 每轮至少换两个还没用过的渠道，不要反复搜同一组关键词。
4. 找到线索先读完整正文、字幕或逐字稿，确认没有登录墙和截断，再用 `add_source.py` 登记，重跑抓取。
5. 读全文终审，按 [编辑判断标准](editorial-judgment.md) 逐题回答是 / 否题。
6. 仍不够就回到第 2 步，直到满足条数或收工检查通过；停下时交缺口报告，不用弱题补位。
