...
<article class="card" data-id="{html.escape(str(item['id']))}">
  <div class="meta"><span>{html.escape(item.get('source_name', '未知来源'))}</span><span>评分 {item['score']}</span></div>
  <h2><a href="{html.escape(item.get('link', '#'))}" target="_blank" rel="noreferrer">{html.escape(item['title'])}</a></h2>
